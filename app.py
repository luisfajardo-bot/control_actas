...
def _sub_run_id() -> str:
    # id corto, único por ejecución
    return f"{int(time.time())}_{uuid.uuid4().hex[:8]}"

def _limpiar_estado_subcontratos():
    # Borra resultados previos (solo Subcontratos)
    for k in ["info_subcontratos", "sub_run_id", "sub_proyecto", "sub_destino"]:
    ...
    if info_sub:
        ...
        with tab_r:
            ...
            if base_general_path.exists():
                ...
                st.dataframe(formatear_numeros_df(df_base.tail(300)), use_container_width=True)
            else:
